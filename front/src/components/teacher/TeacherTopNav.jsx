...
const TeacherTopNav = () => {
  ...
  return (
    <header className="sticky top-0 z-30 bg-gray-800/95 backdrop-blur-md shadow-sm border-b border-slate-700/30">
      <div className="flex items-center justify-between p-4">
        {/* Left Section */}
        <div className="flex items-center gap-4">
          {/* Sidebar Toggle */}
          <button
            className="hidden md:flex p-2 rounded-xl hover:bg-indigo-500/20 text-indigo-400 transition-all duration-300"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <button className="p-2 rounded-xl bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 transition-colors duration-300" title="Create Assignment">
              <i className="ri-file-add-line text-lg" />
            </button>
            <button className="p-2 rounded-xl bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 transition-colors duration-300" title="Grade Submissions">
          ...
          <div className="relative">
            ...
            {showNotifications && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="absolute right-0 mt-2 w-80 bg-gray-800 rounded-xl shadow-lg border border-slate-700/30 z-50"
              >
                ...
                <div className="max-h-64 overflow-y-auto">
                  {notifications.map((notification) => (
                    <div key={notification.id} className="p-4 border-b border-slate-700/30 hover:bg-gray-700/50 transition-colors">
                      <div className="flex items-start gap-3">
                        <div className={`p-2 rounded-lg bg-${notification.color}-500/20`}>
                          <i className={`${notification.icon} text-${notification.color}-400`} />
                        </div>
          ...
          {/* Teacher Profile */}
          <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-indigo-500/20 transition-colors duration-300">
